...
async def handle_sticker(client: Client, message: Message):
    await process_message(client, message, random.choice(smileys))

@Client.on_message(filters.text & filters.private & ~filters.me & ~filters.bot)
async def gchat(client: Client, message: Message):
    await process_message(client, message, message.text.strip())

async def process_message(client, message, user_message):
    try:
        ...
        async def process_combined_messages():
            ...
            chat_history = get_chat_history(user_id, bot_role, combined_message, message.from_user.first_name or "User")
            await send_typing_action(client, message.chat.id, combined_message)

            response = await generate_gemini_response("\n".join(chat_history), chat_history, user_id)
            if not await handle_voice_message(client, message.chat.id, response):
                await message.reply_text(response)

        client.message_timers[user_id] = asyncio.create_task(process_combined_messages())

    except Exception as e:
        await client.send_message("me", f"An error occurred in the `process_message` function:\n\n{str(e)}")

@Client.on_message(filters.private & ~filters.me & ~filters.bot)
async def handle_files(client: Client, message: Message):
    await process_media_message(client, message)

async def process_media_message(client, message):
    try:
        user_id = message.from_user.id
        if user_id in disabled_users or (not gchat_for_all and user_id not in enabled_users):
            return
        ...
        chat_history = get_chat_history(user_id, bot_role, caption, message.from_user.first_name or "User")
        chat_context = "\n".join(chat_history)

        if message.photo:
            await handle_image(client, message, chat_context, caption)
        else:
            file_type, file_path = await get_file_type_and_path(client, message)
            if file_path and file_type:
                uploaded_file = await upload_file_to_gemini(file_path, file_type)
                response = await generate_gemini_response([f"{chat_context}\n\nUser has sent a {file_type}. Caption: {caption if caption else ''} Generate a response based on the content of the {file_type} and our chat context and always follow bot role.", uploaded_file], chat_history, user_id)
                await message.reply(response, reply_to_message_id=message.id)

    except Exception as e:
        await client.send_message("me", f"An error occurred in the `process_media_message` function:\n\n{str(e)}")
    finally:
        if file_path and os.path.exists(file_path):
            os.remove(file_path)

async def handle_image(client, message, chat_context, caption):
    ...
    if client.image_timers[user_id] is None:
        async def process_images():
            ...
            input_data = [f"{chat_context}\n\nUser has sent multiple images. Caption: {caption if caption else ''} Generate a response based on the content of the images and our chat context. Always follow the bot role and talk like a human."] + sample_images
...
async def get_file_type_and_path(client, message):
    if message.video or message.video_note:
        return "video", await client.download_media(message.video or message.video_note)
    if message.audio or message.voice:
        return "audio", await client.download_media(message.audio or message.voice)
    if message.document and message.document.file_name.endswith(".pdf"):
        return "pdf", await client.download_media(message.document)
    if message.document:
        return "document", await client.download_media(message.document)
    return None, None

@Client.on_message(filters.command(["gchat", "gc"], prefix) & filters.me)
async def gchat_command(client: Client, message: Message):
    await handle_gchat_command(client, message)

async def handle_gchat_command(client, message):
    try:
        parts = message.text.strip().split()
        command, user_id = parts[1].lower(), int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else message.chat.id

        if command == "on":
            await set_gchat_status(user_id, enable=True)
            await message.edit_text(f"<b>gchat is enabled for user {user_id}.</b>")
        elif command == "off":
            await set_gchat_status(user_id, enable=False)
        ...
        await client.send_message("me", f"An error occurred in the `handle_gchat_command` function:\n\n{str(e)}")

async def set_gchat_status(user_id, enable):
    if enable:
...
async def set_custom_role(client: Client, message: Message):
    await handle_custom_role_command(client, message)

async def handle_custom_role_command(client, message):
    try:
        parts = message.text.strip().split()
        custom_role, user_id = " ".join(parts[2:]).strip(), int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else message.chat.id
        ...
        await client.send_message("me", f"An error occurred in the `handle_custom_role_command` function:\n\n{str(e)}")

@Client.on_message(filters.command("setgkey", prefix) & filters.me)
async def set_gemini_key(client: Client, message: Message):
    await handle_gemini_key_command(client, message)

async def handle_gemini_key_command(client, message):
    try:
        ...
        if subcommand == "add" and key:
            ...
            await set_gemini_key_index(key, gemini_keys, current_key_index)
        elif subcommand == "del" and key:
            await delete_gemini_key(key, gemini_keys, current_key_index)
        else:
            await display_gemini_keys(message, gemini_keys, current_key_index)

        await asyncio.sleep(1)
    except Exception as e:
        await client.send_message("me", f"An error occurred in the `handle_gemini_key_command` function:\n\n{str(e)}")

async def set_gemini_key_index(key, gemini_keys, current_key_index):
    index = int(key) - 1
...
async def delete_gemini_key(key, gemini_keys, current_key_index):
    index = int(key) - 1
    if 0 <= index < len(gemini_keys):
...
async def display_gemini_keys(message, gemini_keys, current_key_index):
