...
# Default bot role and database collection for chat history
default_bot_role = (
    ...
    "Write short, casual messages like a real human. your response should be a maximum of half line, if message contains only emoji then reply to emoji with emoji and reply to short message with short[...]
    ...
    "Don't send voice messages (only send if they ask), don't use emojis, and rarely use person's name in message so it can feel like human, you only have telegram and instagram (username: @xzrcix) on[...]
...
chat_history_collection = "custom.gchat"
config_collection = "custom.gchat_config"

# Database initialization
enabled_users = db.get(config_collection, "enabled_users") or []
disabled_users = db.get(config_collection, "disabled_users") or []
gchat_for_all = db.get(config_collection, "gchat_for_all") or False

# List of random smileys
smileys = ["-.-", "):", ":)", "*.*", ")*"]

def get_chat_history(user_id, bot_role, user_message, user_name):
    chat_history = db.get(chat_history_collection, f"chat_history.{user_id}") or [f"Role: {bot_role}"]
    chat_history.append(f"{user_name}: {user_message}")
    db.set(chat_history_collection, f"chat_history.{user_id}", chat_history)
    return chat_history

async def generate_gemini_response(input_data, chat_history, user_id):
    retries = 3
    gemini_keys = db.get(config_collection, "gemini_keys") or [gemini_key]
    current_key_index = db.get(config_collection, "current_key_index") or 0
    ...
    while retries > 0:
        try:
            ...
            if "429" in str(e) or "invalid" in str(e).lower():
                ...
                db.set(config_collection, "current_key_index", current_key_index)
...
async def gchat(client: Client, message: Message):
    try:
        ...
        bot_role = db.get(chat_history_collection, f"custom_roles.{user_id}") or default_bot_role
    ...
    try:
        ...
        if command == "on":
            if user_id in disabled_users:
                ...
                db.set(config_collection, "disabled_users", disabled_users)
            if user_id not in enabled_users:
                enabled_users.append(user_id)
                db.set(config_collection, "enabled_users", enabled_users)
...
async def set_custom_role(client: Client, message: Message):
    try:
        ...
        if not custom_role:
            db.set(chat_history_collection, f"custom_roles.{user_id}", default_bot_role)
            db.set(chat_history_collection, f"chat_history.{user_id}", None)
            await message.edit_text(f"Role reset to default for user {user_id}.")
        else:
            db.set(chat_history_collection, f"custom_roles.{user_id}", custom_role)
            db.set(chat_history_collection, f"chat_history.{user_id}", None)
...
async def set_gemini_key(client: Client, message: Message):
    try:
        ...
        gemini_keys = db.get(config_collection, "gemini_keys") or []
        current_key_index = db.get(config_collection, "current_key_index") or 0

        if subcommand == "add" and key:
            gemini_keys.append(key)
            db.set(config_collection, "gemini_keys", gemini_keys)
